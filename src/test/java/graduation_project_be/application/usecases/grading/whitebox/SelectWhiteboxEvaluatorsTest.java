...
class SelectWhiteboxEvaluatorsTest {
    ...
    private static final List<Case> CASES = List.of(
            ...
            new Case("FORBIDDEN_SUBQUERY_IN_SELECT", "{}",
                    "SELECT (SELECT MAX(y) FROM b) AS max_y FROM a",
                    "SELECT id FROM a"),
            new Case("FORBIDDEN_SUBQUERY_IN_FROM", "{}",
                    "SELECT d.id FROM (SELECT id FROM a) d",
                    "SELECT id FROM a"),
            new Case("FORBIDDEN_SUBQUERY_IN_WHERE", "{}",
                    "SELECT id FROM a WHERE x > (SELECT MAX(y) FROM b)",
                    "SELECT id FROM a WHERE x > 0"),
            new Case("FORBIDDEN_SUBQUERY_IN_HAVING", "{}",
                    "SELECT id FROM a GROUP BY id HAVING COUNT(*) > (SELECT AVG(x) FROM b)",
